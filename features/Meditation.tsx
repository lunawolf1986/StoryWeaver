...
const Meditation: React.FC<{
    ...
    onLoadComplete: () => void;
}> = ({ onSave, narrativeToLoad, onLoadComplete }) => {
  const { addTask, tasks } = useTasks();
  const [focus, setFocus] = useLocalStorage<MeditationFocus>('meditation-focus', 'Mindfulness');
  const [targetWordCount, setTargetWordCount] = useLocalStorage<number>('meditation-word-count', 500);
  const [voice, setVoice] = useLocalStorage<string>('med-voice', 'Kore');
  
  const [genre, setGenre] = useLocalStorage<StoryGenre>('med-genre', 'None (General)');
  const [genre2, setGenre2] = useLocalStorage<StoryGenre>('med-genre2', 'None (General)');
  const [genre3, setGenre3] = useLocalStorage<StoryGenre>('med-genre3', 'None (General)');
  const [fandom1, setFandom1] = useLocalStorage<StoryFandom>('med-fandom1', 'None (General)');
  const [fandom2, setFandom2] = useLocalStorage<StoryFandom>('med-fandom2', 'None (General)');
  
  const [customPrompt, setCustomPrompt] = useState('');

  const [generatedText, setGeneratedText, wasTextRestored] = useAutosaveTextarea('autosave-meditation', '');
  ...
  const isBusy = activeTaskCount > 0 || isAnalyzing || isGeneratingAudio || audioPlayerProps.isLoading;

  const currentLengthIndex = useMemo(() => {
    const idx = MEDITATION_LENGTHS.findIndex(l => l.wordCount === targetWordCount);
    if (idx !== -1) return idx;
    
    let closestIdx = 0;
    let minDiff = Infinity;
    MEDITATION_LENGTHS.forEach((l, i) => {
  ...
  const currentLengthName = useMemo(() => {
      const exact = MEDITATION_LENGTHS.find(l => l.wordCount === targetWordCount);
      if (exact) return exact.name;
      if (targetWordCount < 500) return "Quick Prep";
      if (targetWordCount < 1500) return "Standard Journey";
      return "Deep Immersion";
  }, [targetWordCount]);

  useEffect(() => {
    const runningTask = tasks.find(t => t.type === 'generate-meditation' && t.status === 'running');
    if (runningTask && runningTask.partialResult) {
        setGeneratedText(runningTask.partialResult);
    } else if (runningTask === undefined && tasks.find(t => t.type === 'generate-meditation' && t.status === 'completed')) {
        const lastCompleted = tasks.filter(t => t.type === 'generate-meditation' && t.status === 'completed')[0];
  ...
  const handleConfirmReset = useCallback(() => {
    ...
    setFocus('Mindfulness'); setTargetWordCount(500);
  ...
  }, [setGeneratedText, setFocus, setTargetWordCount, setGenre, setGenre2, setGenre3, setFandom1, setFandom2, setIsCustomLength, unloadAudio]);

  useEffect(() => {
    if(narrativeToLoad && narrativeToLoad.type === 'Meditation') {
  ...
  const handleGenerateScript = () => {
    ...
    setCurrentAudioBase64(null);
    addTask({
        type: 'generate-meditation',
        name: `Meditation: ${focus}`,
        payload: { focus, wordCount: targetWordCount, customPrompt, fandom1: [genre, genre2, genre3], fandom2: [fandom1, fandom2] }
    });
  };
  ...
  const handleSave = () => {
    if (!generatedText) return;
    onSave(generatedText, 'Meditation', `Meditation: ${customPrompt.substring(0, 30) || focus}`, undefined, undefined, currentAudioBase64 || undefined);
  };

  const getDurationEstimate = (words: number) => {
      const min = Math.ceil(words / 120);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="bg-gray-800/80 backdrop-blur-sm border border-gray-700 p-6 rounded-xl shadow-lg space-y-6">
        <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-indigo-300">Guided Meditation</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label className="block text-[10px] text-gray-500 uppercase font-bold tracking-widest mb-1">Base Focus</label>
                <select value={focus} onChange={e => setFocus(e.target.value as MeditationFocus)} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-sm text-white">
                  {MEDITATION_FOCUSES.map(f => <option key={f} value={f}>{f}</option>)}
                </select>
            </div>
            <div>
        ...
        <div className="bg-gray-900/40 p-4 rounded-xl border border-gray-700/50 space-y-3">
            <div className="flex justify-between items-center mb-1">
                <div className="flex items-center gap-2">
                    <label className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Session Duration</label>
                    <button 
                ...
                {isCustomLength ? (
                    <div className="flex items-center gap-3 w-full animate-fade-in">
                        <input
                            ...
                            onChange={e => setTargetWordCount(Math.max(50, parseInt(e.target.value) || 50))}
                    ...
                    <input
                        type="range"
                        min="0"
                        max={MEDITATION_LENGTHS.length - 1}
                        step="1"
                        value={currentLengthIndex}
                        onChange={e => setTargetWordCount(MEDITATION_LENGTHS[parseInt(e.target.value)].wordCount)}
                        className="flex-grow h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-indigo-500 animate-fade-in"
                    />
                )}
                <span className="px-2 py-0.5 bg-emerald-900/40 border border-emerald-700/50 rounded text-[10px] font-bold text-emerald-300 uppercase shrink-0">Est. {getDurationEstimate(targetWordCount)} Session</span>
            </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            <div className="space-y-2">
                <label className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Theme / Genre Blend</label>
            ...
            <div className="space-y-2">
                <label className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Atmosphere Settings</label>
        ...
        <TextArea value={customPrompt} onChange={e => setCustomPrompt(e.target.value)} placeholder="Describe the vibe, visual elements, or specific goals for this session..." rows={2}/>
        <div className="text-center pt-2">
          <Button onClick={handleGenerateScript} disabled={isBusy} className="text-lg px-8 py-4 bg-indigo-600 shadow-xl shadow-indigo-900/40">Generate Script</Button>
        </div>
      </div>
      ...
      <div className="bg-gray-800/80 backdrop-blur-sm border border-gray-700 p-6 rounded-xl shadow-lg space-y-4">
        ...
        {(audioPlayerProps.isReady || isGeneratingAudio || audioPlayerProps.isLoading || audioPlayerProps.error || isMp3BackgroundEncoding) && (
            <div className="mt-4">
                <AudioPlayer 
                    ...
                    loadingText={isGeneratingAudio ? "Generating Studio Narration..." : "Preparing Session Audio..."}
                    onSeek={seek}
        ...
        <div className="flex justify-center flex-wrap gap-4 pt-4">
          <Button onClick={handleAnalyzeScript} disabled={!generatedText || isBusy} className="bg-teal-600">Analyze</Button>
      ...
      {analysisResult && (
        <AnalysisModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} suggestions={analysisResult} originalContent={generatedText} onApplyChanges={(txt) => { setGeneratedText(txt); setIsModalOpen(false); }} title="Meditation Analysis" />
      )}
